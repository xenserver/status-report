...
1. **Discovery**:
   ...
   2. The global `data` dictionary with the supported collection directives.
      ...
      For each collection directive, plugin executes
      ...
      These functions populate this global `data` dictionary.
...
collect metadata about the data collection capabilites of xen-bugtool
and its plugins.
...
In the selection phase, some or all discovered capabilties are selected
for collection.
...
2. Run the commands of the selected capabilities and archieve their output.
...
4. Archive the files of the selected capabiltities.
...
flowchart TD
  ...
  A1 --> A2@{ shape: docs,      label: "For all commands of the
                                        selected capabilties
                                        in the <tt>data</tt> dictionary"}
  A2 --> B1@{ shape: processes, label: "Fork a process for each command to run"}
  A2 --> B2@{ shape: processes, label: "Fork a process for each command to run"}
  A2 --> B3@{ shape: processes, label: "Fork a process for each command to run"}
  B1 --> C1@{ shape: bow-rect,  label: "Add the data to the archive"}
  B2 --> C2@{ shape: bow-rect,  label: "Add the data to the archive"}
  B3 --> C3@{ shape: bow-rect,  label: "Add the data to the archive"}
  C1 --> D@{  shape: hourglass, label: "Fork a process for each command to run"}
  C2 --> D
  C3 --> D
  D  --> E["Traverse the <tt>&lt;directory></tt> entries
